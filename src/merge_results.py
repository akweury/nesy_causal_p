...
def merge_principle(res_path, principle, file_lists):
    cont_names = sorted(get_all_subfolder_names(config.get_raw_patterns_path(False) / principle / "train"))
    merged_cont_json = {}
    for start, end in file_lists:
        acc_1 = load_csv(res_path / principle / f"internVL3_78B_{principle}_{start}_{end}_acc.csv")[f"{start}_{end} - {principle}/test_accuracy"].values
        f1_1 = load_csv(res_path / principle / f"internVL3_78B_{principle}_{start}_{end}_f1_score.csv")[f"{start}_{end} - {principle}/f1_score"].values
        precision_1 = load_csv(res_path / principle / f"internVL3_78B_{principle}_{start}_{end}_precision.csv")[f"{start}_{end} - {principle}/precision"].values
        recall_1 = load_csv(res_path / principle / f"internVL3_78B_{principle}_{start}_{end}_recall.csv")[f"{start}_{end} - {principle}/recall"].values
