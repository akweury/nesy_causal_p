...
#                     if model_name == "GRM":
#                         group_analysis[value].append(task_res["accuracy"] * 100)
#                     else:
...
#                 principle_categories[principle] = ["s", "m", "l"]
#             else:
...
#             results[principle] = {}
#             for model_name, model_info in model_dict.items():
#                 results[principle][model_name] = {"related": []}
#                 json_path = analysis_utils.get_results_path(args.remote, principle, model_info["model"], model_info["img_num"])
#                 per_task_data = get_per_task_data(json_path, principle)
#                 rel_acc = []
#                 for task_name, task_res in per_task_data.items():
...
#                     if prop in task_info["related_concepts"]:
#                         if model_name == "GRM":
#                             rel_acc.append(task_res["accuracy"] * 100)
#                             results[principle][model_name]["related"].append(task_res["accuracy"] * 100)
#                         else:
#                             rel_acc.append(task_res["accuracy"])
#                             results[principle][model_name]["related"].append(task_res["accuracy"])
#         for col_idx, principle in enumerate(all_principles):
...
#                     value = np.mean(results[principle][model_name][cat])
#                     ax.bar(x[j] * group_width + i * bar_width, value, width=bar_width, color=palette[j], label=model_name if i == 0 else "")
#                     ax.text(x[j] * group_width + i * bar_width, value + 2, f"{value:.1f}", rotation=60, ha='center', va='bottom', fontsize=10)
#             ax.set_xticks(x * group_width + (n * bar_width) / 2)
#             ax.set_xticklabels(model_names, fontsize=30, ha='right', rotation=30)
#             ax.set_xlabel(prop, fontsize=25)
#             if row_idx == 0:
...
#         rel_vals = avg_results[prop]
#         for i, v in enumerate(rel_vals):
#             ax.bar(x[i], v, width=bar_width, color=palette[i], label=model_names[i])
#             ax.text(x[i], v + 2, f"{v:.1f}", ha='center', va='bottom', fontsize=15, rotation=30)
#         ax.set_title(f"Acc: {prop}", fontsize=30, fontweight='bold')
#         if idx == 0:
#             ax.set_ylabel("Accuracy", fontsize=25)
#         ax.set_xticks(x)
...
#         for col_idx, principle in enumerate(all_principles):
#             ax = axes[row_idx, col_idx] if n_props > 1 else axes[col_idx]
#             has_data = any(len(results[principle][model_name]["related"]) > 0 for model_name in official_model_names)
#             if not has_data:
#                 ax.set_xticks([])
#                 ax.set_yticks([])
#                 ax.axis('off')
...
#             for i, cat in enumerate(categories):
#                 for j, model_name in enumerate(official_model_names):
#                     value = np.mean(results[principle][model_name][cat]) if results[principle][model_name][cat] else np.nan
#                     ax.bar(x[j] * group_width + i * bar_width, value, width=bar_width, color=palette[j], label=model_name if i == 0 else "")
#                     if not np.isnan(value):
#                         ax.text(x[j] * group_width + i * bar_width, value + 2, f"{value:.1f}", rotation=30, ha='center', va='bottom', fontsize=10)
#             ax.set_xticks(x * group_width + (n * bar_width) / 2)
#             ax.set_xticklabels(model_names, fontsize=30, ha='right', rotation=30)
#             ax.set_xlabel(prop, fontsize=25)
#             if row_idx == 0:
