...
#     ax.set_xticklabels(props, fontsize=20)
#     for label, angle in zip(ax.get_xticklabels(), angles[:-1]):
#         label.set_rotation(angle * 180 / pi - 90)  # Rotate tangentially
#         label.set_horizontalalignment('center')
#         label.set_verticalalignment('center')
#
#     ax.xaxis.set_tick_params(pad=18)  # Increase pad to move labels outward
#     grid_steps = 5
#     ring_vals = [min_val + i * (max_val - min_val) / grid_steps for i in range(1, grid_steps + 1)]
#     ax.set_yticks(ring_vals)
#     ax.set_yticklabels([f"{int(v)}" for v in ring_vals], fontsize=14)
#     ax.yaxis.grid(True)
#     ax.xaxis.grid(True)
#
...
#     ax.set_title(title, pad=20, fontsize=22)
#     ax.legend(
#         loc="lower center",
#         bbox_to_anchor=(0.5, -0.18),
#         ncol=len(models_to_values),
#         fontsize=18
#     )
#     fig.tight_layout()
#     fig.savefig(out_pdf_path, format="pdf", bbox_inches="tight")
#     plt.close(fig)
#     print(f"Radar plot saved to {out_pdf_path}")
#     return out_pdf_path
...
def plot_pentagon_radar(
    ...
    # 轴与刻度
    ax.set_xticks([n / float(len(props)) * 2 * pi for n in range(len(props))])
    ax.set_xticklabels(props, fontsize=24)
    for label, angle in zip(ax.get_xticklabels(), angles[:-1]):
    ...
    ring_vals = [min_val + i * (max_val - min_val) / grid_steps for i in range(1, grid_steps + 1)]
    ax.set_yticks(ring_vals)
    ax.set_yticklabels([f"{int(v)}" for v in ring_vals], fontsize=18)
    ax.yaxis.grid(True)
    ax.xaxis.grid(True)
...
if __name__ == "__main__":
    demo_models = {
        ...
        "I-VL3": [62.6, 65.7, 64.4, 62.8, 62.6],
    ...
    plot_pentagon_radar(
        ...
        properties=("size", "color", "shape", "grp_num", "grp_size"),
