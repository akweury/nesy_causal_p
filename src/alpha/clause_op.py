...
def inv_pred_merge_bodies(bodies):
    ...
    arity = len(dtypes)
    inv_pred = InventedPredicate('_'.join(pred_names), arity, dtypes, pred_names)
...
def merge_clauses(clauses, lang):
    ...
    if len(vars_in_body) == 1:
        ...
        inv_atoms = lang.generate_inv_atoms(inv_pred, vars)
        head = clauses[0].head
        for inv_atom in inv_atoms:
            body = trivial_body + [inv_atom]
            merged_clauses.append(Clause(head, body))

    return merged_clauses, inv_atoms
