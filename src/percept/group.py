...
class Group():
    def __init__(self, id, name, input_signal, onside_signal, memory_signal, parents, conf):
        ...
        self.conf = conf
        self.color = self.search_color(parents, input_signal, onside_signal)
        self.ocm = None
        self.pos = None
    ...
    def search_color(self, parents, input_signal, onside_signal):
        if parents is None:
            ...
            most_frequent = color_counts[0][0]  # Get the most frequent color
    ...
    def generate_tensor(self):
        if self.parents == None:
            ...
            parent_size = len(self.parents)
            self.ocm = torch.stack([self.obj2tensor(self.parents[p_i].name,
                                        self.parents[p_i].color,
                                        self.parents[p_i].pos,
                                        self.parents[p_i].name,
                                        self.parents[p_i].conf) for p_i in range(parent_size)])
