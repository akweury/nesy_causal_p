...
def run_vit(args):
    ...
    rtpt = RTPT(name_initials='JIS', experiment_name=f'GRM-vit-{principle}', max_iterations=len(pattern_folders))
    rtpt.start()
    ...
    results_path = Path(output_dir) / f"{model_name}_eval_principle_{principle}_{timestamp}_img_num_{img_num}_start_{start_num}_task_num_{task_num}.json"
